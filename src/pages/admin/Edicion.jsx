...
export const Edicion = () => {
  ...
  const handleEliminarProyecto = async (proyectoId) => {
    ...
    if (confirmacion) {
      try {
        await fetch(`https://api-omega-amber.vercel.app/proyectos/${proyectoId}`, {
          method: 'DELETE',
        });
  ...
  const handleEliminarRubrica = async (rubricaId) => {
    ...
    if (confirmacion) {
      try {
        await fetch(`https://api-omega-amber.vercel.app/rubricas/${rubricaId}`, {
          method: 'DELETE',
        });
    ...
    try {
      ...
      const rubricasWithCasos = await Promise.all(
        rubricasData.map(async (rubrica) => {
          const casosResponse = await fetch(`https://api-omega-amber.vercel.app/casos?rubricaId=${rubrica.id}`);
        ...
        rubricasWithCasos.map(async (rubrica) => {
          const casosWithPuntos = await Promise.all(
            rubrica.casos.map(async (caso) => {
              const puntosResponse = await fetch(`https://api-omega-amber.vercel.app/puntos?casoId=${caso.id}`);
  ...
  const fetchCategorias = async () => {
    try {
      const response = await fetch('https://api-omega-amber.vercel.app/categorias');
      const data = await response.json();
      setCategorias(data);
    } catch (error) {
